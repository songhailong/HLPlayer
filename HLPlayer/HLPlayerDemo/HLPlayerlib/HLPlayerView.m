...
- (void)setOrientationLandscape
{
    if (self.isCellVideo) {
        ...
        // 亮度view加到window最上层
        HLBrightnessView *brightnessView = [HLBrightnessView sharedBrightnessView];
...
- (void)doubleTapAction:(UITapGestureRecognizer *)gesture
{
    // 显示控制层
    [self animateShow];
    [self palystartAction:self.controlView.startBtn];
}

-(void)palystartAction:(UIButton *)button{
...
- (void)animateShow
{
    ...
    [UIView animateWithDuration:ZFPlayerControlBarAutoFadeOutTimeInterval animations:^{
        ...
        //[]
        [UIApplication sharedApplication].statusBarHidden=NO;
        //UIViewController *vcself=[self hl_getviewController];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if (object == self.player.currentItem) {
        if ([keyPath isEqualToString:@"status"]) {
            ...
            if (self.playeIndex==0) {
                self.playeIndex++;
                self.state=ZFPlayerStateFirst;
            }
...
{    NSLog(@"=====状态%zd",state);
    ...
    // 控制菊花显示、隐藏
    state == ZFPlayerStateBuffering ? ([self.controlView.activityView startAnimating]) : ([self.controlView.activityView stopAnimating]);
}
...
- (void)moviePlayDidEnd:(NSNotification *)notification
{
    ...
    if (self.isBottomVideo && !self.isFullScreen) { // 播放完了，如果是在小屏模式 && 在bottom位置，直接关闭播放器
        ...
        self.controlView.backgroundColor  = HLColor(0, 0, 0, .6);
        self.playDidEnd                   = YES;
...
-(void)lockScreenAction:(UIButton *)lockScreenBtn{
    lockScreenBtn.selected             = !lockScreenBtn.selected;
    self.isLocked               = lockScreenBtn.selected;
    // 调用AppDelegate单例记录播放状态是否锁屏，在TabBarController设置哪些页面支持旋转
    ZFPlayerShared.isLockScreen = lockScreenBtn.selected;
}
/**全屏按钮点击事件*/
-(void)fullScreenAction:(UIButton *)fullScreenBtn{
    ...
    if (self.isCellVideo && fullScreenBtn.selected == YES) {
        [self interfaceOrientation:UIInterfaceOrientationPortrait];
        return;
    }
 ...
 *  返回当前视图的控制器
 */
- (UIViewController *)hl_getviewController {
...
#pragma mark - 控制层 ZFPlayerControlView
 ...
 */
- (HLPlayerControlView *)controlView
{
    if (!_controlView) {
        _controlView = [[HLPlayerControlView alloc] init];
